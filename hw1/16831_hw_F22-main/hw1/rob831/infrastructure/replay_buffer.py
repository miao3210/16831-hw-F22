...
class ReplayBuffer(object):
    ...
    def sample_random_data(self, batch_size):
        ...
        # miao
        total = self.obs.shape[0]
        indices = np.random.permutation(total)
        #numpy.take(a, indices, axis)
        sample_obs = np.take(a=self.obs, indices=indices[:batch_size], axis=0)
        sample_acs = np.take(a=self.acs, indices=indices[:batch_size], axis=0)
        sample_rews = np.take(a=self.rews, indices=indices[:batch_size], axis=0)
        sample_next_obs = np.take(a=self.next_obs, indices=indices[:batch_size], axis=0)
        sample_terminals = np.take(a=self.terminals, indices=indices[:batch_size], axis=0)
