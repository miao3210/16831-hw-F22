...
class FFModel(nn.Module, BaseModel):
    ...
    def forward(
        ...
        next_obs_pred = unnormalize(delta_pred_normalized, delta_mean, delta_std) + obs_unnormalized # TODO(Q1)
    ...
    def get_prediction(self, obs, acs, data_statistics):
        ...
        #print(acs.shape)
        with torch.no_grad():
            prediction, _ = self(obs_unnormalized=obs,
    ...
    def update(self, observations, actions, next_observations, data_statistics):
        ...
        _, predicted_obs = self(obs_unnormalized=observations,
        ...
        target = normalize(delta, self.delta_mean, self.delta_std)
        
        loss = self.loss(predicted_obs, target) # TODO(Q1) compute the loss
        # Hint: `self(...)` returns a tuple, but you only need to use one of the
        # outputs.
