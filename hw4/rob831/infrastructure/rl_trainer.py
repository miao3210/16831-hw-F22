...
class RL_Trainer(object):
    ...
    def __init__(self, params):
        ...
        if 'unar' in self.params['env_name'] and 'ander' in self.params['env_name']:
            self.env = gym.make(
                "LunarLander-v2",
                continuous = False,
                gravity = -10.0,
            ...
            self.env.action_space.low = self.env.action_space.start
            self.env.action_space.high = self.env.action_space.start + self.env.action_space.n - 1
            #import pdb 
            #pdb.set_trace()
        else:
...
#        print('Collecting train data...')
    ...
    def perform_logging(self, itr, paths, eval_policy, train_video_paths, all_logs):
        ...
        eval_paths, eval_envsteps_this_batch = utils.sample_trajectories(self.env, eval_policy, self.params['eval_batch_size'], self.params['ep_len'], eval=True)
        print('miao add: finish eval data collecting')
