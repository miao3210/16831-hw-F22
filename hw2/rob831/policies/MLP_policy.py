...
class MLPPolicy(BasePolicy, nn.Module, metaclass=abc.ABCMeta):
    ...
    def get_action(self, obs: np.ndarray) -> np.ndarray:
        ...
        # miao 
        #return ptu.to_numpy(output) # AttributeError: 'Categorical' object has no attribute 'to'
...
class MLPPolicyPG(MLPPolicy):
    ...
    def update(self, observations, actions, advantages, q_values=None):
        ...
        logprob = output.log_prob(actions)
        policy_loss = - torch.sum( logprob * advantages ) # miao tried mean, but q1 failed autograder # miao used to use torch.sum
        ...
        if self.nn_baseline:
            ...
            #import pdb
            #pdb.set_trace()
            #normalizer = nn.LayerNorm(q_values.shape[0])
            #normalized_q_values = normalizer(ptu.to(q_values))
            #normalized_q_values = ptu.from_numpy((q_values - q_values.mean()) / np.sqrt(q_values.var()))
            ...
            baseline_loss = self.baseline_loss(pred, normalized_q_values)
