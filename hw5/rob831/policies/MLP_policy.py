...
class MLPPolicy(BasePolicy, nn.Module, metaclass=abc.ABCMeta):
    ...
    def forward(self, observation: torch.FloatTensor):
        if self.discrete:
            ...
            ## miao
            #nprint(self.optimizer.state_dict())
            if (torch.isnan(logits)).max():
                import pdb 
                pdb.set_trace()
...
class MLPPolicyAWAC(MLPPolicy):
    ...
    def update(self, observations, actions, adv_n=None, q_fn=None):
        if adv_n is None:
            assert False
        ...
        probs = ac_dist.log_prob(actions)
        actor_loss = - (probs * (1 / self.lambda_awac * adv_n).exp() + 1e-10).sum() # torch.tensor([-104.]).exp() is 0. this is risky
        #print(actor_loss)
        #if actor_loss == 0:
        #    import pdb 
        #    pdb.set_trace()
