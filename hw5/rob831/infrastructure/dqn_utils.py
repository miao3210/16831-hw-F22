...
def register_custom_envs():
    ...
    if 'PointmassEasy-v0' not in registry.env_specs:
        register(
            id='PointmassEasy-v0',
            entry_point='rob831.envs.pointmass.pointmass:Pointmass',
            kwargs={'difficulty': 0}
        )
...
class MemoryOptimizedReplayBuffer(object):
    ...
    def _encode_sample(self, idxes):
        ...
        ret_batch = self.returns[idxes]

        return obs_batch, act_batch, rew_batch, next_obs_batch, done_mask, ret_batch
    ...
    def store_frame(self, frame):
        ...
        if self.obs is None:
            ...
            self.returns   = np.empty([self.size],                     dtype=np.float32)
    ...
    def store_effect(self, idx, action, reward, done):
        ...
        if idx == 0:
            ...
            self.returns[idx] = (self.returns[idx-1] * (1-self.done[idx-1])) + reward

        i=1
        while (idx-i > 0) and (not self.done[idx-i]):
            self.returns[idx-i] = self.returns[idx]
            i += 1
