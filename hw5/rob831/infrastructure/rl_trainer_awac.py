...
class RL_Trainer(object):
    ...
    def run_training_loop(self, n_iter, collect_policy, eval_policy,
        ...
        for itr in range(n_iter):
            ...
            if isinstance(self.agent, AWACAgent):
                if (not self.agent.offline_exploitation) or (self.agent.t <= self.agent.num_exploration_steps):
                    print('add paths, t={}, thresh={}'.format(self.agent.t, self.agent.num_exploration_steps), end='\r')
                    self.agent.add_to_replay_buffer(paths)
                else:
                    print('dont add paths, t={}, thresh={}'.format(self.agent.t, self.agent.num_exploration_steps), end='\r')
    ...
    def train_agent(self):
        ...
        for train_step in range(self.params['num_agent_train_steps_per_iter']):
            ob_batch, ac_batch, re_batch, next_ob_batch, terminal_batch, ret_batch = self.agent.sample(self.params['train_batch_size'])
            train_log = self.agent.train(ob_batch, ac_batch, re_batch, next_ob_batch, terminal_batch, ret_batch)
