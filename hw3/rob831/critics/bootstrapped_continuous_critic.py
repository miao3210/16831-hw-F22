...
class BootstrappedContinuousCritic(nn.Module, BaseCritic):
    ...
    def update(self, ob_no, ac_na, next_ob_no, reward_n, terminal_n):
        ...
        ## miao why there is an ac_na in input
        ...
        for nt in range(self.num_target_updates):
            with torch.no_grad():
                value_tp1 = self.forward(next_ob_no)
                value_target = value_tp1 * (1-terminal_n)
                value_target = value_target * self.gamma 
                value_target = reward_n + value_target
            #target = value_target.detach()
            for ng in range(self.num_grad_steps_per_target_update):
                self.optimizer.zero_grad()
                value_t = self.forward(ob_no)
                loss = self.loss(value_t, value_target)
                loss.backward()
                self.optimizer.step()
        #print('1500 value_t={}, value target={}'.format(value_t[1500], value_target[1500]))
        return loss.item()