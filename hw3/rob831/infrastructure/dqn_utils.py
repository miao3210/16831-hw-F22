...
def get_env_kwargs(env_name):
    if env_name in ['MsPacman-v0', 'PongNoFrameskip-v4']:
        ...
        kwargs = {
            ...
            'learning_freq': 1,
            'frame_history_len': 1,
            'target_update_freq': 3000, #100, # hparam3 10000, # hparam2 300, # hparam1 1000, # original 3000,
            'grad_norm_clipping': 10,
            'lander': True,
...
class invunet(nn.Module):
    ...
    def forward(self, obs):
        ...
        f128 = self.l2(f64)
        f256 = self.l3(f128)
        f256 = f256 + self.l4(f256)
        f128 = f128 + self.l5(f256)
        f64 = f64 + self.l6(f128)
        out = self.l7(f64)
        return out

def create_lander_q_network(ob_dim, num_actions):
    #return invunet(ob_dim, num_actions) 
    '''
...
def lander_optimizer():
    return OptimizerSpec(
        constructor=optim.Adam,
        optim_kwargs=dict(
            lr=1, # original 1, hparam1 0.1, hparam2 0.01, hparam3 0.001
        ),
